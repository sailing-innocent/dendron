...
export type PreSetupHookFunction<T = any> = (opts: WorkspaceOpts & {
    extra?: any;
}) => Promise<T>;
...
export type PreSetupCmdHookFunction<T = any> = (opts: Pick<WorkspaceOpts, "wsRoot">) => Promise<T>;
export type PostSetupHookFunction<T = any> = (opts: {
    engine: DEngineClient;
    extra?: any;
} & WorkspaceOpts) => Promise<T>;
 ...
 * ```
 */
export type SinonStubbedFn<T extends (...args: any[]) => any> = SinonStub<Parameters<T>, ReturnType<T>>;
